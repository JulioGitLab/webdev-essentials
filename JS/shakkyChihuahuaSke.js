...
const drawStSc = function () {
  ...
  // eyes
  fill(color.WhiteSmoke);
  circle(sX - 37, sY - 13, sW * 0.31);
  circle(sX + 37, sY - 13, sW * 0.31);
  fill(color.FireBrick);
  circle(sX - 36, sY - 10, sW * 0.19);
  circle(sX + 36, sY - 10, sW * 0.19);
  // muzzle
  ...
  endShape();
  ellipse(sX, sY + 23, sW * 0.19, sW * 0.14);
  // tongue
  fill(color.LightCoral);
  ...
  arc(mX - 16, mY + 21, 31, 23, 0, PI * 0.8); // PI * DEG/180
  arc(mX + 16, mY + 21, 31, 23, PI * 0.2, PI); // PI * DEG/180
  // ears
  beginShape();
  vertex(mX - 73, mY - 25);
...
const drawPlaySc = function () {
  ...
  // more Meowwys
  if (meowwy.x < -50 && toys[toys.length - 1].x > 750) {
...
const meowwy = new Cat(
  Math.floor(Math.random() * 750 + 750), // from 750 to 1500
  Math.floor(Math.random() * 70 + 50) // from 50 to 120
);

let toys = [];
let chanclas = 0;
/* // TODO: Check p5.js random()
for (let i = 0; i < 40; i++) {
